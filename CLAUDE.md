...
- **Flow 판에 AI 프로필을 따로 세우지 않는다.** 대리인은 주인 노드에 접어 넣고,
  AI가 날랐다는 것은 **주황 선**으로만 말한다. 노드를 하나 더 세우면 사람 다섯인
  회의가 노드 여덟이 되고, 정작 읽어야 하는 "누가 누구에게"가 대리인 이름에 묻힌다.
  - Discord AI와 서비스 AI도 구분하지 않는다 — 둘 다 그 사람의 대리인일 뿐이다.
  - 사람이 자기 대리인과 주고받은 것은 그릴 선이 없으므로 **그 사람 노드에 주황
    테두리 뱃지**로 붙인다. 버리면 "내가 내 대리인에게 지시한 기록"이 사라진다.
